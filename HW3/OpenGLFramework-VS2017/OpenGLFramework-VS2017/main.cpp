...
GLint shininess = 64;
GLint iLocMV, iLocKa, iLocKd, iLocKs, iLocDir, iLocPos, iLocView_pos, iLocMode;
...
GLint iLocIsEye , iLocEyeX, iLocEyeY;
...
void phong_input(int mtl_index, Matrix4 V, int shade_mode)
{
	...
	glUniform1f(iLocEyeX, x);
	glUniform1f(iLocEyeY, y);
	glUniform1i(iLocIsEye, models[cur_idx].shapes[mtl_index].material.isEye);
...
void RenderScene(int per_vertex_or_per_pixel) {
	...
	{
		phong_input(i, view_matrix, per_vertex_or_per_pixel);
		/*
		...
		*/
		...
		if (magnification_fliter == 0) 
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		else if (magnification_fliter == 1) 
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		if (minification_fliter == 0) 
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		else if (minification_fliter == 1) 
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, models[cur_idx].shapes[i].material.diffuseTexture);
		glUniform1i(glGetUniformLocation(program, "TextureMapping"), 0);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (action == GLFW_PRESS) {
		...
		case 'J':
			...
			break;
		case GLFW_KEY_G:
			magnification_fliter = (magnification_fliter + 1) % 2;
			break;
		case GLFW_KEY_B:
			minification_fliter = (minification_fliter + 1) % 2;
			break;
		case GLFW_KEY_RIGHT:
			...
			//cout<<(models[cur_idx].cur_eye_offset_idx);
			break;
		case GLFW_KEY_LEFT:
			if (models[cur_idx].cur_eye_offset_idx == 0)
				models[cur_idx].cur_eye_offset_idx = 6;
			else
				models[cur_idx].cur_eye_offset_idx = (models[cur_idx].cur_eye_offset_idx - 1) % models[cur_idx].max_eye_offset;
			//cout << (models[cur_idx].cur_eye_offset_idx);
			break;
...
void LoadTexturedModels(string model_path)
{
	...
	//cout << "has eye " << tmp_model.hasEye << endl;
...
void setUniformVariables()
{
	...
	iLocTextureMapping = glGetUniformLocation(program, "TextureMapping");
	iLocIsEye = glGetUniformLocation(program, "isEye");
	iLocEyeX = glGetUniformLocation(program, "EyeX");
	iLocEyeY = glGetUniformLocation(program, "EyeY");

	//hw2
	iLocDir = glGetUniformLocation(program, "light_dir");
	iLocPos = glGetUniformLocation(program, "light_pos");
	iLocMode = glGetUniformLocation(program, "cur_light_mode");
	iLocIntensity = glGetUniformLocation(program, "intensity");
	iLocShininess = glGetUniformLocation(program, "shininess");
	iLocAngle = glGetUniformLocation(program, "angle");
	iLocShademode = glGetUniformLocation(program, "shade_mode");
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109065535 HW3", NULL, NULL);
    if (window == NULL)
    {
