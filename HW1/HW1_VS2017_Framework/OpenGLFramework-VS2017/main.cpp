...
void setPerspective()
{
	//p132
	GLfloat f;
	f = -1 / tan(proj.fovy / 2);//-1 / tan(proj.fovy / 2);cot(proj.fovy / 2) 不加負號方向上下顛倒??
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case GeoRotation:
		...
		break;
	case ViewEye:
		main_camera.position -= Vector3(0, 0, 0.001 * yoffset);
		setViewingMatrix();
		break;
	case ViewCenter:
		main_camera.center -= Vector3(0, 0, 0.001 * yoffset);
		setViewingMatrix();
		break;
	case ViewUp:
		main_camera.up_vector -= Vector3(0, 0, 0.001 * yoffset);
		setViewingMatrix();
		break;
	}
}
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (mouse_pressed == true) {
		...
		case GeoRotation:
			...
			break;
		case ViewEye:
			main_camera.position += Vector3(0.001 * (xpos - starting_press_x), 0.001 * (ypos - starting_press_y), 0);
			setViewingMatrix();
			break;
		case ViewCenter:
			...
			break;
		case ViewUp:
			main_camera.up_vector += Vector3(0.001 * (xpos - starting_press_x), 0.001 * (ypos - starting_press_y), 0);
			setViewingMatrix();
			break;
		}
	}
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109065535 HW1", NULL, NULL);
    if (window == NULL)
    {
