...
GLint shininess = 64;
GLint iLocKa, iLocKd, iLocKs;
GLint iLocLight_pos, iLocIntensity, iLocView_pos, iLocLightMode, iLocShademode, iLocAngle, iLocShininess;
GLuint iLocTextureMapping;
int magnification_fliter = 0;
int minification_fliter = 0;
...
void phong_input(int mtl_index, int shade_mode)
{
	/*
	glUniform3f(iLocView_pos, main_camera.position[0], main_camera.position[1], main_camera.position[2]);
	glUniform3f(iLocKa, models[cur_idx].shapes[mtl_index].material.Ka[0], models[cur_idx].shapes[mtl_index].material.Ka[1], models[cur_idx].shapes[mtl_index].material.Ka[2]);
	glUniform3f(iLocKd, models[cur_idx].shapes[mtl_index].material.Kd[0], models[cur_idx].shapes[mtl_index].material.Kd[1], models[cur_idx].shapes[mtl_index].material.Kd[2]);
	glUniform3f(iLocKs, models[cur_idx].shapes[mtl_index].material.Ks[0], models[cur_idx].shapes[mtl_index].material.Ks[1], models[cur_idx].shapes[mtl_index].material.Ks[2]);
	glUniform1i(iLocShademode, shade_mode);
	glUniform1i(iLocLightMode, cur_light_mode);
	glUniform1f(iLocShininess, shininess);

	if (cur_light_mode == 0) {
		glUniform3f(iLocLight_pos, directional.position[0], directional.position[1], directional.position[2]);
		glUniform3f(iLocIntensity, directional.intensity[0], directional.intensity[1], directional.intensity[2]);
	}
	if (cur_light_mode == 1) {
		glUniform3f(iLocLight_pos, point.position[0], point.position[1], point.position[2]);
		glUniform3f(iLocIntensity, point.intensity[0], point.intensity[1], point.intensity[2]);
	}
	if (cur_light_mode == 2) {
		glUniform3f(iLocLight_pos, spot.position[0], spot.position[1], spot.position[2]);
		glUniform3f(iLocIntensity, spot.intensity[0], spot.intensity[1], spot.intensity[2]);
		glUniform1f(iLocAngle, spot.angle);
	...
	*/
	...
	glUniform3f(iLocView_pos, main_camera.position[0], main_camera.position[1], main_camera.position[2]);
	glUniform3f(iLocLight_pos, light_tmp.position[0], light_tmp.position[1], light_tmp.position[2]);
	glUniform3f(iLocIntensity, light_tmp.intensity[0], light_tmp.intensity[1], light_tmp.intensity[2]);
...
void setShaders()
{
	...
	iLocM = glGetUniformLocation(p, "um4m");
	...
	//hw2
	iLocLight_pos = glGetUniformLocation(p, "light_pos");
	iLocView_pos = glGetUniformLocation(p, "view_pos");
	iLocLightMode = glGetUniformLocation(p, "cur_light_mode");
